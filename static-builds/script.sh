...
find ${DISTNAME}/bin -type f -executable -exec ../clorecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../clorecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo clore-*.tar.gz`
mkdir -p ~/clorecoin/temp
cd ~/clorecoin/temp
tar xf ../$SOURCEDIST
find clore-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/clorecoin
...
cp -f clore-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
rm -rf win64
cp -rf clorecoin/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/clorecoin/clore-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf clorecoin/contrib/windeploy ~/sign/win32
...
mkdir -p ~/clorecoin/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/clorecoin/depends/SDKs/MacOSX10.11.sdk.tar.gz
...
cd ~/clorecoin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Clore-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
