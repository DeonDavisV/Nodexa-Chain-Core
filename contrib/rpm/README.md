...
The RPM spec file provided here is for Clore-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://blockchain.clore.ai/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Clore git repository](https://gitlab.com/cloreai-public/blockchain) but are not part of
...
Currently only `Source10` is used, for the example `clore.conf` file.
...
in the clore git repository. At present only an SVG version of the Clore
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the clore
...
needed when 0.12.1 is released, a proper fix is already in the Clore git
master branch.
...
idea to build Clore Core against a static version of that library compiled
...
The `clore.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `clore-server` package. At this point, most RPM based
...
The `clore-server` package should have SELinux support. How to properly do
...
Clore-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/clore/). He has been
packaging Clore for Fedora at least since 2012.
...
4. I renamed the `clore` package that contains the Qt GUI to `clore-core` as
that appears to be how the general population refers to it, in contrast to
`clore-xt` or `clore-classic`. I wanted to make sure the general population
