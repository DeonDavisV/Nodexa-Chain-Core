...
UniValue UpdateAddressTag(const JSONRPCRequest &request, const int8_t &flag)
{
    ...
    if (!IsValidDestination(destination)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Clore address: ") + address);
    }
    ...
    if (request.params.size() > 2) {
        ...
        if (!change_address.empty()) {
           ...
           if (!IsValidDestination(change_dest)) {
               throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Clore change address: ") + change_address);
           }
        }
    }
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. \"change_address\"        (string), optional, default=\"\"), address the the clore change will be sent to, if it is empty, change address will be generated for you\n"
        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                                   std::string("Invalid Change Address: Invalid Clore address: ") + change_address);
            }
        }
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "Five (5) CLORE will be burned for each asset created.\n"
                ...
                "5. \"change_address\"        (string, optional, default=\"\"), address the the clore change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Change Address: Invalid Clore address: ") + changeAddress);
        }
    }
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a clore address\n"
...
UniValue transfer(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
                ...
                "6. \"change_address\"       (string, optional, default = \"\") the transactions CLORE change will be sent to this address\n"
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Clore address: ") + to_address);
    }
    ...
    std::string clore_change_address = "";
    if (request.params.size() > 5) {
        clore_change_address = request.params[5].get_str();
    }
    ...
    CTxDestination clore_change_dest = DecodeDestination(clore_change_address);
    if (!clore_change_address.empty() && !IsValidDestination(clore_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("CLORE change address must be a valid address. Invalid address: ") + clore_change_address);
    ...
    CCoinControl ctrl;
    ctrl.destChange = clore_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddresses \"asset_name\" [\"from_addresses\"] qty \"to_address\" \"message\" expire_time \"clore_change_address\" \"asset_change_address\"\n"
            + AssetActivationWarning() +
            ...
            "7. \"clore_change_address\"       (string, optional, default = \"\") the transactions CLORE change will be sent to this address\n"
    ...
    std::string clore_change_address = "";
    if (request.params.size() > 6) {
        clore_change_address = request.params[6].get_str();
    }
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
                "transferfromaddress \"asset_name\" \"from_address\" qty \"to_address\" \"message\" expire_time \"clore_change_address\" \"asset_change_address\"\n"
                + AssetActivationWarning() +
                ...
                "5. \"message\"                  (string, optional) Once HIP5 is voted in ipfs hash or txid hash to send along with the transfer\n"
                "6. \"expire_time\"              (numeric, optional) UTC timestamp of when the message expires\n"
                "7. \"clore_change_address\"       (string, optional, default = \"\") the transaction CLORE change will be sent to this address\n"
...
UniValue listtagsforaddress(const JSONRPCRequest &request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid CLORE address: ") + address);
...
UniValue checkaddresstag(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() != 2)
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                "1. \"address\"          (string, required) the CLORE address to search\n"
...
UniValue issuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 4 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "5. \"change_address\"        (string, optional, default=\"\") address that the clore change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue reissuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "6. \"change_address\"        (string, optional, default=\"\") address that the clore change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Clore address: ") + change_address);
        }
    }
...
#ifdef ENABLE_WALLET
    { "assets",   "transferfromaddress",        &transferfromaddress,        {"asset_name", "from_address", "qty", "to_address", "message", "expire_time", "clore_change_address", "asset_change_address"}},
    { "assets",   "transferfromaddresses",      &transferfromaddresses,      {"asset_name", "from_addresses", "qty", "to_address", "message", "expire_time", "clore_change_address", "asset_change_address"}},
