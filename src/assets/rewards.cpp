...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for CLORE
    ...
    if (p_rewardSnapshot.strDistributionAsset != "CLORE") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is CLORE with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using CLORE differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "CLORE") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Clore address (already validated during ownership snapshot creation)
