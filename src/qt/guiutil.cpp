...
#include "cloreaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a clore address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new CloreAddressEntryValidator(parent));
    widget->setCheckValidator(new CloreAddressCheckValidator(parent));
}
...
bool parseCloreURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no clore: URI
    if(!uri.isValid() || uri.scheme() != QString("clore"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CloreUnits::parse(CloreUnits::CLORE, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCloreURI(QString uri, SendCoinsRecipient *out)
{
    // Convert clore:// to clore:
    //
    //    Cannot handle this later, because clore:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("clore://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "clore:");
    }
    QUrl uriInstance(uri);
    return parseCloreURI(uriInstance, out);
}

QString formatCloreURI(const SendCoinsRecipient &info)
{
    QString ret = QString("clore:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CloreUnits::format(CloreUnits::CLORE, info.amount, false, CloreUnits::separatorNever));
        paramCount++;
    }
...
bool openCloreConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CLORE_CONF_FILENAME));

    /* Open clore.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Clore.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Clore (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Clore (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "clore.desktop";
    return GetAutostartDir() / strprintf("clore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a clore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Clore\n";
        else
            optionFile << strprintf("Name=Clore (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the clore app
...
bool GetStartOnSystemStartup()
{
    CFURLRef cloreAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (cloreAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, cloreAppUrl);

    CFRelease(cloreAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add clore app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, cloreAppUrl, nullptr, nullptr);
    }
