...
const std::string CloreGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString CloreGUI::DEFAULT_WALLET = "~Default";

CloreGUI::CloreGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void CloreGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a clore address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and clore: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to CLORE addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your clore addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified clore addresses"));
    ...
    openAction->setStatusTip(tr("Open a Clore: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible clore command-line options").arg(tr(PACKAGE_NAME)));
...
void CloreGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/clorecointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* cloreLabelLayout = new QVBoxLayout(toolbarWidget);
        cloreLabelLayout->addWidget(label);
        cloreLabelLayout->addWidget(toolbar);
        cloreLabelLayout->setDirection(QBoxLayout::TopToBottom);
        cloreLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Clore Market Price"));
        ...
        QLabel* labelBtcClore = new QLabel();
        labelBtcClore->setText("BTC / CLORE");
        labelBtcClore->setContentsMargins(15,0,0,0);
        labelBtcClore->setFixedHeight(75);
        labelBtcClore->setAlignment(Qt::AlignVCenter);
        labelBtcClore->setStyleSheet(STRING_LABEL_COLOR);
        labelBtcClore->setFont(currentMarketFont);*/
       ...
       /* priceLayout->setGeometry(headerWidget->rect());
        ...
        priceLayout->addWidget(labelBtcClore, 0 , Qt::AlignVCenter | Qt::AlignLeft);
        ...
        // Get the latest Clore release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void CloreGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool CloreGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool CloreGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void CloreGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CloreGUI::setWalletActionsEnabled(bool enabled)
{
...
void CloreGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void CloreGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void CloreGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CloreGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void CloreGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void CloreGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void CloreGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void CloreGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void CloreGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void CloreGUI::openClicked()
{
...
void CloreGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CloreGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CloreGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CloreGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CloreGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CloreGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** CLORE START */
void CloreGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void CloreGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void CloreGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void CloreGUI::gotoRestrictedAssetsPage()
{
...
void CloreGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to clore network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void CloreGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CloreGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void CloreGUI::updateHeadersSyncProgressLabel()
{
...
void CloreGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void CloreGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Clore"); // default title
...
void CloreGUI::changeEvent(QEvent *e)
{
...
void CloreGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CloreGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CloreGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "CLORE")
        msg += tr("Amount: %1\n").arg(CloreUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(CloreUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void CloreGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CloreGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CloreGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CloreGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CloreGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void CloreGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CloreGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CloreGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CloreGUI::detectShutdown()
{
...
void CloreGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void CloreGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void CloreGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CloreGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(CloreGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void CloreGUI::subscribeToCoreSignals()
{
...
void CloreGUI::unsubscribeFromCoreSignals()
{
...
void CloreGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void CloreGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<CloreUnits::Unit> units = CloreUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CloreUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(CloreUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(CloreUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (CloreUnits::Unit u : CloreUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(CloreUnits::name(u)), this);
...
{
    setText(CloreUnits::name(newUnits));
}
...
void CloreGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=CLOREBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void CloreGUI::mnemonic()
{
...
void CloreGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/CloreChain/Clore/releases"));
