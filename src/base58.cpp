...
class CCloreAddressVisitor : public boost::static_visitor<bool>
{
private:
    CCloreAddress* addr;

public:
    explicit CCloreAddressVisitor(CCloreAddress* addrIn) : addr(addrIn) {}
...
bool CCloreAddress::Set(const CKeyID& id)
{
    SetData(GetParams().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CCloreAddress::Set(const CScriptID& id)
{
...
bool CCloreAddress::IsValid() const
{
    return IsValid(GetParams());
}

bool CCloreAddress::IsValid(const CChainParams& params) const
{
...
CTxDestination CCloreAddress::Get() const
{
    if (!IsValid())
...
bool CCloreAddress::GetIndexKey(uint160& hashBytes, int& type) const
{
    if (!IsValid()) {
        return false;
...
void CCloreSecret::SetKey(const CKey& vchSecret)
{
...
bool CCloreSecret::IsValid() const
{
...
bool CCloreSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CCloreSecret::SetString(const std::string& strSecret)
{
...
{
    return CCloreAddress(str).IsValid(params);
}
