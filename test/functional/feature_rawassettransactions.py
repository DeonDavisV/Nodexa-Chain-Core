...
class RawAssetTransactionsTest(CloreTestFramework):
    ...
    def reissue_tampering_test(self):
        ...
        tx.deserialize(f)
        clorer = '72766e72'  # clorer
        op_drop = '75'
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if clorer in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                reissue_script_hex = script_hex[script_hex.index(clorer) + len(clorer):-len(op_drop)]
                ...
                tampered_script = script_hex[:script_hex.index(clorer)] + clorer + tampered_reissue + op_drop
        ...
        f = BytesIO(hex_str_to_bytes(tx_hex))
        tx.deserialize(f)
        cloret = '72766e74'  # cloret
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: cloret not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_hex_bad = bytes_to_hex_str(tx.serialize())
        tx_signed = n0.signrawtransaction(tx_hex_bad)['hex']
    ...
    def issue_tampering_test(self):
        ...
        tx.deserialize(f)
        cloreo = '72766e6f'  # cloreo
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: cloreo not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        ...
        owner_vout = list(filter(lambda out_script: cloreo in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))[0]
        ...
        f = BytesIO(hex_str_to_bytes(tx_issue_hex))
        tx.deserialize(f)
        cloreq = '72766e71'  # cloreq
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: cloreq not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-bad-asset-transaction",
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if cloreo in bytes_to_hex_str(out.scriptPubKey):
                owner_out = out
                owner_script_hex = bytes_to_hex_str(owner_out.scriptPubKey)
                asset_script_hex = owner_script_hex[owner_script_hex.index(cloreo) + len(cloreo):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                owner = CScriptOwner()
                owner.deserialize(f)
                owner.name = b"NOT_MY_ASSET!"
                tampered_owner = bytes_to_hex_str(owner.serialize())
                tampered_script = owner_script_hex[:owner_script_hex.index(cloreo)] + cloreo + tampered_owner + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        ...
        for n in range(0, len(tx.vout)):
            ...
            if cloreo in bytes_to_hex_str(out.scriptPubKey):
                ...
                tampered_script = owner_script_hex.replace(cloreo, HVNO)
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-op-clore-asset-not-in-right-script-location",
                                n0.sendrawtransaction, tx_bad_issue_signed)
    ...
    def issue_reissue_transfer_test(self):
        ...
        for i in range(1, 3):
            ...
            f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(cloret) + len(cloret):-len(op_drop)]))
            ...
            tampered_script = script_hex[:script_hex.index(cloret)] + cloret + tampered_transfer + op_drop
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if cloret in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(cloret) + len(cloret):-len(op_drop)]))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
    ...
    def issue_sub_multiple_outputs_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if cloret in bytes_to_hex_str(out.scriptPubKey):
                ...
                asset_script_hex = transfer_script_hex[transfer_script_hex.index(cloret) + len(cloret):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
                transfer.amount = 0
                tampered_transfer = bytes_to_hex_str(transfer.serialize())
                tampered_script = transfer_script_hex[:transfer_script_hex.index(cloret)] + cloret + tampered_transfer + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
    ...
    def transfer_asset_inserting_tampering_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if cloret in bytes_to_hex_str(out.scriptPubKey):
                ...
                # Generate a script that has a valid destination address but switch it with cloreq and the issue_serialized data
                issue_out.scriptPubKey = hex_str_to_bytes(transfer_script_hex[:transfer_script_hex.index(cloret)] + cloreq + issue_serialized + op_drop)
    ...
    def atomic_swaps_test(self):
        ...
        # n1 buys 400 ANDUIN from n2 for 4000 CLORE
        price = 4000
        amount = 400
        fee = 0.01
        ...
        # n2 buys JAINA! from n1 for 20000 CLORE
        price = 20000
        amount = 1
    ...
    def fundrawtransaction_transfer_outs(self):
        ...
        for _ in range(0, 5):
            n0.sendtoaddress(n2_address, clore_amount / 5)
        n0.generate(1)
        self.sync_all()
