...
"""
Clore test framework primitive and message structures.
...
COIN = 100000000  # 1 clore in Corbies
...
class CAddress:
    """
    Objects that map to clore_blockchaind objects, which can be serialized/deserialized
    """
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in clore_blockchaind
            if flags != 0:
                self.vin = deser_vector(f, CTxIn)
...
class MsgHeaders:
    ...
    def deserialize(self, f):
        # comment in clore_blockchaind indicates these should be deserialized as blocks
