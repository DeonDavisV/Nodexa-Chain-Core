...
class CloreTestFramework:
    """Base class for a clore test script.

    Individual clore test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def start_nodes(self, extra_args=None):
        """Start multiple clore_blockchainds"""
    ...
    def stop_node(self, i):
        """Stop a clore_blockchaind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple clore_blockchaind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("CloreRPC")
