...
`test/functional/test_framework/test_framework.py:CloreTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/clore_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    logging.debug("Temporary test directory at %s" % tmpdir)

    # Don't run tests on Windows by default
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://gitlab.com/cloreai-public/blockchain/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/Bitcoin/bitcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    # Check that the build was configured with wallet, utils, and clore_blockchaind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_clore_blockchaind = config["components"].getboolean("ENABLE_CLORE_BLOCKCHAIND")
    if not (enable_wallet and enable_cli and enable_clore_blockchaind):
        print("No functional tests to run. Wallet, utils, and clore_blockchaind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if clore_blockchaind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "clore_blockchaind"]) is not None:
            print("%sWARNING!%s There is already a clore_blockchaind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "CLORE_BLOCKCHAIND" not in os.environ:
        os.environ["CLORE_BLOCKCHAIND"] = build_dir + '/src/clore_blockchaind' + exeext
        os.environ["CLORECLI"] = build_dir + '/src/clore-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `clore-cli help` (`rpc_interface.txt`).
